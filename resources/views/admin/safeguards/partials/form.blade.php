@php
    ...
    $selectedCategory = old('category', $safeguard?->category ?? request('category'));
@endphp

<div class="row g-3">
    <div class="col-md-8">
        <label class="form-label">Safeguard section</label>
        <select name="category" class="form-select" required>
            <option value="" disabled @selected(! $selectedCategory)>Choose section…</option>
    ...
    <div class="col-md-4">
        ...
        <input type="number" name="sort_order" min="0" max="65535" class="form-control" value="{{ old('sort_order', $safeguard?->sort_order ?? 0) }}">
...
<div class="mb-3 mt-3">
    ...
    <input type="text" name="title" class="form-control" value="{{ old('title', $safeguard?->title) }}" required>
    @error('title')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
</div>

<div class="mb-3">
    <label class="form-label">Description</label>
    <textarea name="description" class="form-control" rows="6" placeholder="Summary shown on the public Safeguard table…">{{ old('description', $safeguard?->description) }}</textarea>
...
<div class="row g-3">
    <div class="col-md-8">
        ...
        @if ($safeguard?->document_path)
            <div class="alert alert-light border mt-2 mb-0 py-2 small">
                <strong>Current:</strong> {{ $safeguard->document_original_name ?: basename($safeguard->document_path) }}
                <span class="badge text-bg-secondary ms-1">{{ $safeguard->documentTypeLabel() }}</span>
    ...
    <div class="col-md-4">
        ...
        <input type="date" name="document_date" class="form-control" value="{{ old('document_date', $safeguard?->document_date?->format('Y-m-d')) }}">
...
<div class="mb-3 mt-3">
    ...
    @if ($safeguard && count($safeguard->images ?? []) > 0)
        <div class="irdc-admin-safeguard-existing-images mb-3">
            @foreach ($safeguard->images as $imagePath)
...
<div class="mb-3">
    ...
    <select name="status" class="form-select">
        <option value="published" @selected(old('status', $safeguard?->status ?? 'published') === 'published')>Published</option>
        <option value="draft" @selected(old('status', $safeguard?->status) === 'draft')>Draft</option>
    </select>
