...
<section class="border-b border-stone-200/90 bg-gradient-to-b from-stone-50 to-white">
    <div class="mx-auto max-w-6xl px-4 py-10 text-center sm:py-14">
        <h1 class="font-display text-3xl font-extrabold leading-tight tracking-tight sm:text-4xl md:text-[2.4rem]">
            <span class="text-slate-800">IRDCRP</span>
            <span class="text-[#6B2B27]"> {{ __('messages.nav_media_photos') }}</span>
        </h1>
...
<section
    class="mx-auto max-w-7xl px-3 py-8 sm:px-6 sm:py-12"
    x-data="{
...
>
    @forelse($items as $item)
        @if($loop->first)
            <div class="columns-1 gap-4 [column-fill:_balance] sm:columns-2 sm:gap-5 lg:columns-3">
        @endif
        <article class="mb-4 break-inside-avoid group sm:mb-5">
            <div class="overflow-hidden rounded-2xl bg-stone-100 shadow-sm ring-1 ring-stone-200/70 transition duration-300 hover:shadow-lg hover:ring-stone-300/90">
                <button
                    type="button"
                    @click="openAt({{ $loop->index }})"
                    class="block w-full rounded-t-2xl text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2"
                >
    ...
    @empty
        <div class="rounded-2xl border border-dashed border-stone-300 bg-stone-50/80 px-6 py-16 text-center">
            <p class="mx-auto max-w-md text-slate-500">{{ __('messages.gallery_empty') }}</p>
        </div>
    @endforelse
    ...
    <template x-teleport="body">
        ...
        >
            <div
                ...
                @click.stop
            >
                <div class="relative flex max-h-[90vh] min-h-0 items-center justify-center">
                    <button
                        type="button"
                        x-show="items.length > 1"
                        @click="prev()"
                        class="absolute left-0 z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-white/20 sm:left-1 sm:h-12 sm:w-12"
                        aria-label="{{ __('messages.gallery_lightbox_prev') }}"
                    >
                        <span class="text-2xl leading-none" aria-hidden="true">‹</span>
                    </button>
                    <div class="mx-10 flex min-h-0 flex-1 items-center justify-center sm:mx-14">
                        <img
                    ...
                    <button
                        ...
                        x-show="items.length > 1"
                        @click="next()"
                        class="absolute right-0 z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-white/20 sm:right-1 sm:h-12 sm:w-12"
