...
<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" name="sort_order" min="0" max="65535" class="form-control" value="{{ old('sort_order', $report?->sort_order ?? 0) }}">
...
<div class="mb-3 mt-3">
    ...
    <textarea name="description" class="form-control" rows="6" placeholder="Summary shown on the public Reports table…">{{ old('description', $report?->description) }}</textarea>
...
<div class="row g-3">
    <div class="col-md-8">
        ...
        @if ($report?->document_path)
            <div class="alert alert-light border mt-2 mb-0 py-2 small">
                <strong>Current:</strong> {{ $report->document_original_name ?: basename($report->document_path) }}
                <span class="badge text-bg-secondary ms-1">{{ $report->documentTypeLabel() }}</span>
    ...
    <div class="col-md-4">
        ...
        <input type="date" name="document_date" class="form-control" value="{{ old('document_date', $report?->document_date?->format('Y-m-d')) }}">
...
<div class="mb-3 mt-3">
    ...
    @if ($report && count($report->images ?? []) > 0)
...
<div class="mb-3">
    ...
    <select name="status" class="form-select">
        <option value="published" @selected(old('status', $report?->status ?? 'published') === 'published')>Published</option>
