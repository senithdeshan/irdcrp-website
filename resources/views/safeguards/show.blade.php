...
@section('title', $categoryTitle.' | '.config('app.name'))

@section('content')
@php
    $otherCategory = collect(\App\Models\SafeguardResource::CATEGORIES)->except($category);
@endphp

<section class="irdc-safeguard-hero">
    <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        <div class="max-w-3xl">
            <p class="irdc-safeguard-hero__eyebrow">Safeguard</p>
            <h1 class="irdc-safeguard-hero__title">{{ $categoryTitle }}</h1>
            <p class="irdc-safeguard-hero__lead">
                Official safeguard reports, screening documents, supporting images, and downloadable files published by the project team.
            </p>
        </div>
    </div>
</section>
...
<section class="irdc-safeguard-section">
    <div class="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        @if ($otherCategory->isNotEmpty())
            <nav class="irdc-safeguard-switcher" aria-label="Safeguard sections">
                <span class="irdc-safeguard-switcher__label">Sections</span>
                <a
                    href="{{ route('safeguards.show', $category) }}"
                    class="irdc-safeguard-switcher__link is-active"
                    aria-current="page"
                >{{ $categoryTitle }}</a>
                @foreach ($otherCategory as $slug => $label)
                    <a href="{{ route('safeguards.show', $slug) }}" class="irdc-safeguard-switcher__link">{{ $label }}</a>
                @endforeach
            </nav>
        @endif
                        ...
                        <tr>
                            ...
                            <td data-label="Report">
                                @if ($item->documentExists())
                                    <a href="{{ route('safeguards.download', $item) }}" class="irdc-safeguard-download">
                                ...
                                @if (count($imageUrls) > 0)
                                    ...
                                    >
                                        ...
                                        <div
                                            ...
                                            role="dialog"
                                            aria-modal="true"
                                            aria-label="Safeguard image viewer"
                                        >
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="irdc-safeguard-table__empty">
                                No published items in this section yet. Content will appear here once added from the admin Safeguard area.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
